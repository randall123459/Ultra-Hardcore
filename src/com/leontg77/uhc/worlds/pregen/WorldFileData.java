...
public class WorldFileData {
	...
	private transient Map<CoordXZ, List<Boolean>> regionChunkExistence = Collections
			.synchronizedMap(new HashMap<CoordXZ, List<Boolean>>());

	// Use this static method to create a new instance of this class. If null is
	// returned, there was a problem so any process relying on this should be
	// cancelled.
	public static WorldFileData create(World world, Player notifyPlayer) {
		...
		if (!newData.regionFolder.exists()
			...
			// check for region folder inside a DIM* folder (DIM-1 for nether,
			// DIM1 for end, DIMwhatever for custom world types)
		...
		// Accepted region file formats: MCR is from late beta versions through
		// 1.1, MCA is from 1.2+
	...
	// the constructor is private; use create() method above to create an
	...
	public boolean doesChunkExist(int x, int z) {
		...
		// Bukkit.getLogger().info("x: "+x+"  z: "+z+"  offset: "+coordToRegionOffset(x,
		// z));
	...
	// Minecraft only fully generates a chunk when adjacent chunks are also
	// loaded.
	...
	// Method to let us know a chunk has been generated, to update our region
	// map.
	...
	// region is 32 * 32 chunks; chunk pointers are stored in region file at
	...
	private int coordToRegionOffset(int x, int z) {
		// "%" modulus is used to convert potential world coordinates to
		// definitely be local region coordinates
		x = x % 32;
		z = z % 32;
		// similarly, for local coordinates, we need to wrap negative values
		// around
		if (x < 0)
			x += 32;
		if (z < 0)
	...
	private List<Boolean> getRegionData(CoordXZ region) {
		...
		// data for the specified region isn't loaded yet, so init it as empty
		...
		for (int i = 0; i < regionFiles.length; i++) {
			...
			try {
				RandomAccessFile regionData = new RandomAccessFile(
						this.regionFile(i), "r");
				// first 4096 bytes of region file consists of 4-byte int
				// pointers to chunk data in the file (32*32 chunks = 1024; 1024
				// chunks * 4 bytes each = 4096)
				for (int j = 0; j < 1024; j++) {
					// if chunk pointer data is 0, chunk doesn't exist yet;
					// otherwise, it does
